...
import type { CreateLeasePayload, LeaseStatus, Tenant } from "../types";
...
export default function LeaseCreateModal({
  ...
  const canSubmit = useMemo(() => {
    return (
      ...
      unitId > 0 &&
      Boolean(startDate) &&
      Boolean(rentAmount.trim()) &&
      Boolean(primaryTenantId) &&
      !isSaving
    );
  }, [orgSlug, unitId, startDate, rentAmount, primaryTenantId, isSaving]);
  ...
  const handleSubmit = async () => {
    ...
    const dueDay =
      dueDayNum && Number.isFinite(dueDayNum)
        ? Math.min(31, Math.max(1, dueDayNum))
        : null;
    ...
    const payload: CreateLeasePayload = {
      ...
      parties: [
        {
          tenant: Number(primaryTenantId),
          role: "primary",
        },
      ],
    };

    // Step 4: call parent mutation
