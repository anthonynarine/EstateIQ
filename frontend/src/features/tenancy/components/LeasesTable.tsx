 ...
 * - Renders leases for a unit
 * - Shows loading/empty states
 * - Displays parties_detail (tenant + role)
 *
 * Server state is owned by TanStack Query in the page-level container.
 */
export default function LeasesTable({ leases, isLoading, isFetching }: Props) {
  // Step 1: pre-format parties for stable rendering
  const rows = useMemo(() => {
    return leases.map((l) => {
      const parties =
